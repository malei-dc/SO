...
void test_get_block_address(Ext2FS * fs){
    ...
    inodo = fs->load_inode(23673);
    assert(inodo->size == 26051);
    int block_address_5 = fs->get_block_address(inodo,5);
	int block_address_25 = fs->get_block_address(inodo, 25);
    assert(block_address_5 == 60982);
	assert(block_address_25 == 890);
}

void test_get_file_inode_from_dir_inode(Ext2FS * fs){
	cout << "=== Test Get File Inode From Dir ===" << endl;
